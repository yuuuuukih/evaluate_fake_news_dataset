...
def plot_boxplot(data: list, label: list[str] = ['real (before)', 'real (after)', 'fake'], file_name: str = 'boxplot', whis: float = 1.5):
    """箱ひげ図を描画する関数"""
    ...
    for keyword_group in before_sammarized_data['data']:
        for timeline in keyword_group['timeline_info']['data']:
            for doc in timeline['timeline']:
                ...
                if doc_id not in set(manage_doc_ids):
                    manage_doc_ids.append(doc_id)
