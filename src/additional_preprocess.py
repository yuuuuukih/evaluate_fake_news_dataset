...
class AdditionalPreprocessor:
    ...
    """
    def remove_initial_phrase_and_capitalize(self, input_text: str, separator: str = "[SEP] content:"):
    ...
    def test_remove_initial_phrase_and_capitalize(self, separator: str = "[SEP] content:"):
        TEST_DATA = [
            {
        ...
        for test_data in TEST_DATA:
            ...
            actual_output_text = self.remove_initial_phrase_and_capitalize(input_text, separator=separator)
    ...
    def remove_in_phrase(self, do_test: bool = False):
        if do_test:
            self.test_remove_initial_phrase_and_capitalize(separator="[SEP] content:")
            return

        for mode in ['base', 'pre_target_timeline', 'all_timeline', 'around_target']:
            for what in ['train_25', 'train_50', 'train_75', 'train_100', 'val', 'test']:
                file_path = os.path.join(self.root_dir, self.sub_dir, mode, f'{what}.json')
                with open(file_path, 'r') as F:
                    dataset = json.load(F)

                for example in dataset['data']:
                    if example['tgt'] == 1:
                        example['src'] = self.remove_initial_phrase_and_capitalize(example['src'], separator="[SEP] content:")

                save_dir = os.path.join(self.root_dir, self.sub_dir, f'{mode}_no_in')
...
def main():
    ...
    Remove phrase beginning with "In" in the fake news content in the preprocessed dataset.
    """
    ap.remove_in_phrase(do_test=False)
